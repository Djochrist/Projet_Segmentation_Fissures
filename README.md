...
⚠️ **Important** : si le dossier existe déjà dans votre session Colab (message
`fatal: destination path ... already exists`), `git clone` ne fait **rien** —
vous continuez avec une version potentiellement ancienne du code, sans les
derniers correctifs. Utilisez plutôt la cellule ci-dessous, qui clone au
premier lancement et fait un `git pull` sinon :
